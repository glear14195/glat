...
var messageHandler = {
  ...
  getMessagesForGroupSolr: function (gid, latLong, cb) {
    if (gid && latLong) {
      var queryStr = [];
      queryStr.push(`q=type_s:message`);
      queryStr.push(`fq=gid_i:${gid}`);
      ...
      solrClient.search(queryStr, function (err, result) {
        if (err) {
          ...
          var phoneDict = retArr.map((obj) => obj.uid_s);
          userHandler.getNameDict(phoneDict, function (err, nameDict) {
            retArr.map((obj) => obj.createdByName = nameDict[obj.uid_s]);
            console.log(retArr);
  ...
  getMessageFeed: function(gid,mid,cb) {
    if(gid && mid ) {
      var query = `select u.dname,mfeed.comment,mfeed.created_at 
                      from users u join message_feed mfeed on 
                      u.phone=mfeed.uid and mfeed.mid= ${mid} and mfeed.gid= ${gid}
                      order by mfeed.created_at desc `;
...
if (require.main === module) {
  ...
  messageHandler.getMessagesForGroupSolr(24, '-12.2, 22', function (err, res) {
    console.log(err || res);
  });
}
