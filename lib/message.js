...
var messageHandler = {
  ...
  messageLogAddForGid: function (gid, cb) {
    if (gid) {
      var query = `insert into message_log(gid, uid, mid, status) 
      select gm.gid, gm.uid, m.id, (case when gm.uid = m.uid then 2 else 0 end) from message m 
      join group_members gm on gm.gid = m.gid and gm.gid = ${gid} and gm.status = 1 left join message_log ml
      on ml.gid = m.gid and ml.uid = gm.uid and ml.mid = m.id where ml.id is null`;
      pgClient.execute(query, cb);
    } else {
      cb (`Missing parameters`, null);
    }
  ...
  getMessageFeed: function(gid, mid, cb) {
    if(gid && mid) {
      var query = `select u.dname as user,mfeed.comment as message,mfeed.created_at as createdAt,
      case when u.pic_location is not null or u.pic_location != '' 
      then u.pic_location else '' end as pic_location 
      from users u join message_feed mfeed on u.phone=mfeed.uid 
      and mfeed.mid= ${mid} and mfeed.gid= ${gid} order by mfeed.created_at asc `;
  ...
  formatMsgForClient: function (msgObj) {
    if (msgObj) {
      ...
      return {
        body: msgObj.msg_s,
        lat: parseFloat(msgObj.location_p[0].trim()),
        long: parseFloat(msgObj.location_p[1].trim()),
        gid: msgObj.gid_i,
        mid: msgObj.mid_i,
        createdByNum: msgObj.uid_s,
        createdByName: msgObj.createdByName,
        createdAt: butils.timeFormat(msgObj.created_at_dt),
        sensorData: msgObj.sensordata_txt.length ? JSON.parse(msgObj.sensordata_txt[0]) : ``,
