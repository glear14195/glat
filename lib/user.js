  ...
  'getNames': function (users, gid, phone, cb) {
    if (Array.isArray(users) && users.length) {
      ...
      users.forEach(function (number) {
        ...
        if (temp_key && temp_key !== phone) map_dict[temp_key] = number;
      });
      var query = `with a as (select unnest(ARRAY['${Object.keys(map_dict).join("','")}']) as num)
    select a.num as phone,u.dname ${extraCol} from a join users u on u.phone = a.num ${extraCond} order by ${gid ? `is_member desc,` : ``}u.dname ;`
      pgclient.execute(query, function (err, result) {
        if (!err && result) {
          result = result.map((retObj) => ({ phone: map_dict[retObj.phone], dname: retObj.dname, is_member: retObj.is_member || false}));
          cb(null, result);
        } else {
          cb('execution_error');
        }
...
if (require.main === module) {
  (function () {
    ...
    user.getNames(['1234567890', '09185662341', '555666', '9962036295', '8778131717'], ``, function (err, result) {
